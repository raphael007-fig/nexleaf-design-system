...
## 2026-09-03 — Onto the Add-Equipment wizard frame (Raf: "should have the add equipment frame, and be full width")
- **What:** The screen now rides StepFrame (the shared wizard chrome from the Add Equipment flow): full-width fixed-height card, section headers (FormSection), pinned footer actions — matching Prototype C's Add Equipment surface. Bulk import keeps its 3-phase stepper inside the frame with visited-phase tap-back; the preview step widens the content column (new additive `contentMaxWidth` param on StepFrame) so the validation table isn't crushed.
- **Verified:** upload/map/preview + add form render inside the frame in-browser; build passes.
