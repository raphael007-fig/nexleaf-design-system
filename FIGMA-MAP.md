...
| **Add-equipment steps on the board** | **Both flows show every step.** Unmonitored: S1 step 1 · **S1b step 2 (Warranty & Maintenance)** · **S1c step 3 (Review & submit)**. Monitored: F1 step 1 (merged facility + equipment) · **F3 step 2 (the SAME shared Warranty & Maintenance)** · F5/F6 step 3 (base station, sensor plus-rows, alarms) · F7 step 4 review. Steps 1 and 2 are one shared implementation across the two flows — the Sep 2026 meeting's "monitored mirrors unmonitored". | 2026-09-08 |
| **Sensor assignment on the board** | **Plus-rows, not a multi-select.** Each row = serial + ROLE (In-room / Ambient / Door) + Remove, with "+ Add sensor" beneath. A CT5 carries four; a CTX draws from its pre-fed pool. | 2026-09-08 |
| **The stale "AI Chat Bot (beta)" pill** | **Retexted to `Ask AI` on all 70 frames**, matching the DS TopBar and the prototype. Do not hide it — hiding leaves a hole where the pill belongs. | 2026-09-08 |
| **Mobile board** | **Deleted 2026-09-08** (7 sections, 144 children) on Raphael's instruction, to be rebuilt from the finished desktop frames. `I3m` was preserved as `REF · MOBILE ROW ANATOMY` (owner-tagged) because it IS registry node `9629:14571`. | 2026-09-08 |
| **Register KPI row** | **5 cards: Total equipment + the EQUIPMENT STATUS split** (Functional · Faulty · Unknown · Decommissioned), each with a week-on-week pill — "No change" / "N more/fewer than last week", toned by whether the lab wants that movement. Monitored-vs-not is the tab row and the Monitored column, not a KPI. **Supersedes the 4-card KPI clause of `9590:18832` and the 2-up KPI grid of `8483:121543`** — Raphael's ruling when the collision was put to him. | 2026-09-07 |
| **Bulk import on the board** | **Sync it in full from the prototype**, despite the Sep-2026 meeting deprioritising the feature for engineering — the board must not carry a design the prototype has moved past. Unused I frames get cleared. | 2026-09-07 |
